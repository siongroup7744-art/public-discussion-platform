...
function renderSubcategories() {
  ...
  btnAdd.textContent = translations[currentLang].addSubcategory;
  btnAdd.addEventListener('click', () => {
    const name = prompt(translations[currentLang].enterSubcategoryName);
    if (name && name.trim()) {
      category.subcategories = category.subcategories || [];
      category.subcategories.push({ id: createId(), name: name.trim(), topics: [] });
      saveData();
      renderSubcategories();
    }
  });
...
function renderTopics() {
  ...
  const btnAdd = document.createElement('button');
  btnAdd.className = 'btn add-btn';
  btnAdd.textContent = translations[currentLang].addTopic;
  btnAdd.addEventListener('click', () => {
    const title = prompt(translations[currentLang].enterTopicTitle);
    if (title && title.trim()) {
      subcategory.topics = subcategory.topics || [];
      subcategory.topics.push({ id: createId(), title: title.trim(), comments: [] });
      saveData();
      renderTopics();
    }
  });
  ...
  if (!subcategory.topics || !subcategory.topics.length) {
    const li = document.createElement('li');
